...
___________________________________________________________________________________________________________________'''
        ...
        for lroot, ldirs, lfiles in os.walk(self.local_pdf_detail_file):
            for file in lfiles:
                ...
                if (str(file).__contains__('.pdf') or str(file).__contains__('.PDF')) and file_name not in file_name_old:
                    self.file_to_dataframe_pdfplumber(file)
                    # try:
                    # except:
                    #     continue
        # 将pdf结果转成dataFrame
        table_data = pd.DataFrame(
            self.pdf_data_val, columns=self.add_data_title)
    ...
    def file_to_dataframe_pdfplumber(self, fileName):
        # 文件名
