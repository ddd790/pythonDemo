...
class VAS_GUI():
    ...
    def file_to_dataframe(self, io):
        # ---------------------- 劳动合同 ------------------
        excel_header = ['工号', '姓名', '部门', '职务', '性别', '入职时间', '出生日期', '身份证号', '地址', '联系电话']
        # ---------------------- 审核资料-面料TC ------------------
        ...
        for i in range(len(df)):
            ...
            if (df.iloc[i]['性别'] == '女' and now_age < 50) or (df.iloc[i]['性别'] == '男' and now_age < 60):
                context = {
                    ...
                    "职务": df.iloc[i]['职务'],
                    "性别": df.iloc[i]['性别'],
                    "入职时间": str(df.iloc[i]['入职时间']).replace(' 00:00:00', ''),
                    "出生日期": str(df.iloc[i]['出生日期']).replace(' 00:00:00', ''),
                    "身份证号": df.iloc[i]['身份证号'],
                    "地址": df.iloc[i]['地址'],
                    "联系电话": df.iloc[i]['联系电话'],
                }
    ...
    def get_age(self, birthday):
        ...
        r_monthandday = r_monthandday.replace('-', '')[:4]
        # 取出输入日期的月与日
        r_year = birthday[:4]
        # 取出输入日期的年份
