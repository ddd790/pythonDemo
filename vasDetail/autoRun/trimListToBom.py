...
class VAS_GUI():
    ...
    def arrange_appand_by_type(self, arrangeVal, type):
        ...
        if type.__contains__('coats'):
            ...
            arrangeVal.append(['胸兜牌衬（透色面料用前片衬）', '2346-2HE', '黑', '2.1cm', ''])
            arrangeVal.append(['有纺直条/胸衬聚量用', '5850-1', '黑', '2.0cm', ''])
            arrangeVal.append(['端打条（无里子用小涤带）/袖窿一周/后领口（AETA不用端打条，用小涤带）', '5850-3', '黑', '1.2cm', ''])
            arrangeVal.append(['拉丝无纺衬条/止口', '9332-1', '黑', '1.0cm', ''])
            ...
            arrangeVal.append(['裤口袋布/腰里上部/腰里下部/前后兜袋/右前兜内零钱兜（看工艺）/普通裆布/下巾里子/下巾里子连洗水裆布', '全涤人字纹-ECO-8303 / 涤棉人字纹-ECO-4303P', '黑', '146cm', ''])
            arrangeVal.append(['前门襟拉链', 'CFC-36 DA3', '', '', ''])
            arrangeVal.append(['裤钩', 'B498 ', '亮银色', '', ''])
            arrangeVal.append(['裤内扣', 'SB', '黑', '22L', ''])
            arrangeVal.append(['无纺衬/侧斗位/后兜牙/后兜位/探头贴/上巾/下巾', 'PE125 ', '炭灰', '150cm', ''])
            arrangeVal.append(['无纺衬-腰里下部', 'PE125 ', '炭灰', '150cm', ''])
            arrangeVal.append(['腰硬衬/腰面', 'FW6951M68/RP 1515 ', '黑', '3.3cm', ''])
            arrangeVal.append(['腰硬衬/腰面', 'FW6951M68/RP 1515 ', '黑', '150cm', '用于腰衬加工'])
            arrangeVal.append(['绊带衬/绊带', '4947', '黑', '0.9cm', ''])
            arrangeVal.append(['腰网衬/腰里', '6148', '黑', '5.5cm', ''])
            arrangeVal.append(['双面胶', '双面胶', '白', '0.8cm', ''])
            arrangeVal.append(['PL腰里加工', 'PL腰里加工', '', '', ''])
        elif type.__contains__('Vests'):
            arrangeVal.append(['马甲钎子', 'BG87-006JZ', '古铜色', '', ''])
            arrangeVal.append(['兜布/腰兜布/腰兜布拉条/里兜布/外胸兜布（看工艺）', 'ECO-8301', '黑', '146cm', ''])
            arrangeVal.append(['前身衬/前片', 'PE206 ', '黑', '148cm', ''])
            arrangeVal.append(['腰兜牌衬/腰兜牌', '2346-2HE', '黑', '', ''])
            arrangeVal.append(['无纺衬/贴边衬/前袖窿衬/前肩衬/省尖衬/下摆贴/腰兜牙（双牙兜）', 'PE125', '炭灰', '150cm', ''])
            arrangeVal.append(['拉丝衬条/止口（AETA不用）', '9332-1', '黑', '1cm', ''])
            ...
            arrangeVal.append(['拉丝衬/前袖窿上12CM长/前袖窿弯/前下摆圆', 'F0125N/JS6038 ', '黑', '99cm', ''])
            arrangeVal.append(['无胶衬/里兜牙2/手机兜牙1', 'SF-35 ', '黑', '99cm', ''])
            arrangeVal.append(['胸兜牌衬/胸兜牌（透色面料用前片衬）', '2346-2HE ', '黑', '2.1cm', ''])
            arrangeVal.append(['有纺直条/胸衬聚量用', '5850-1 ', '黑', '2.0cm', ''])
            arrangeVal.append(['拉丝无纺衬条/止口', '9332-1 ', '黑', '1.0cm', ''])
            ...
            arrangeVal.append(['口袋布/兜布/腰兜布/兜布拉条（贴兜不用）/贴兜封口条（大贴边不用）/ 里兜布/手机兜布/胸兜布（TALLIA品牌不用，看工艺书） 上衣+马甲', 'ECO-8301', '黑', '146cm', ''])
            arrangeVal.append(['兜位衬/里兜位/手机兜位', '0118N/8025', '黑', '99cm', ''])
            ...
            arrangeVal.append(['有纺直条/胸衬聚量用', '5850-1 ', '黑', '2.0cm', ''])
            arrangeVal.append(['拉丝无纺衬条/上衣止口/马甲止口（AETA不用）', '9332-1 ', '黑', '1.0cm', ''])
