...
___________________________________________________________________________________________________________________'''
        ...
        for root, dirs, files in os.walk(networked_directory):
            if root.__contains__('2026') or root.__contains__('2027'):
                for file in files:
    ...
    def get_deliver_to(self, pdfreader):
        for index in range(len(pdfreader.pages)):
            ...
            # 找到"Please deliver to:"的top坐标和词索引
            deliver_to_top = None
            label_word_indices = set()
            for i, w in enumerate(words):
                if w['text'] == 'Please' and i + 2 < len(words) and \
                   words[i + 1]['text'] == 'deliver' and \
                   words[i + 2]['text'].startswith('to'):
                    deliver_to_top = w['top']
                    label_word_indices = {i, i + 1, i + 2}
                    break
            if deliver_to_top is None:
                continue
            # 筛选出同线及下方的words,排除"Please deliver to:"这三个词本身
            below_words = sorted(
                [w for i, w in enumerate(words) if i not in label_word_indices and w['top'] >= deliver_to_top - 1],
            ...
            for line_idx, line_words in enumerate(line_groups[:6]):
                ...
                for w in line_sorted[1:]:
                    if w['x0'] - groups[-1][-1]['x1'] > 15:
                        ...
                        groups[-1].append(w)
                # 第1行必取(左段可能被Please deliver to:标签占据只剩2段);第2行及以上只取有左中右3段结构的行
                if line_idx > 0 and len(groups) < 3:
                    continue
                if len(groups) >= 3:
                    middle = groups[1]
                else:
                    middle = groups[0]
                text = ' '.join(w['text'] for w in middle)
                if text:
